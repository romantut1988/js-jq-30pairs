...
// !Эти слова воспринимаются интерпретатором как команды, выражения, предназначенные для выполнения. Соответственно, 
// такие слова нельзя использовать для других нужд, например, в качестве имен переменных или функций. Хоть это и не 
// запрещено, но крайне не рекомендуется использовать слова, отличающиеся от ключевых только регистром символов — «FOR», 
// «Var» или «typeOf».




// Переменные. Правила именования переменных важнейшей частью программирования, послужившей когда-то причиной перехода 
// от первого ко второму поколению языков, являются переменные. Для того чтобы использовать переменную в программе 
// необходимо дать ей имя.
// Строгие правила обязательны для исполнения, нарушение их приведет к ошибкам выполнения программы.
// Для имен переменных запрещается использование ключевых или зарезервированных слов (см. раздел «Ключевые и 
// зарезервированные слова»). ■ В именах переменных не допускаются пробельные и разделительные символы (табуляция, 
// пробел, запятая, точка с запятой и т.п.). ■ Имена переменных не могут содержать символы операций (математических, 
// логических, битовых, — всех), например «+», «!» или «&». Особое внимание следует обратить на то, что знак минуса «–», 
// часто используемый для имен классов в HTML, для имен переменных в JavaScript не разрешен. ■В именах переменных не 
// допускаются специальные символы операторов, например «.», «{}» или «?:», а также кавычки всех типов. Чем перечислять 
// все подобные символы, проще сказать, что из них в именах переменных допускаются только «_» (нижнее подчеркивание) и 
// «$» (знак доллара).Имя переменной не может начинаться с цифры (но может содержать цифры в середине или в конце).




// Типы данных.
// Переменная — это именованная область памяти, предназначенная для хранения некоторых данных.
...
Стандартом установлено, что этот тип имеет ровно 18437736874454810627 различных значений, представляющих 
число двойной точности в формате 64 бит.  */
...
// Основными типами, предназначенными для работы с данными, являются «Number» (для работы с числами), «String» 
// (для работы с текстами) и «Object» (для группировки данных). Типы данных «Null» и «Undefined» используются для 
// проверки успешного выполнения операций и для хранения данных не применяются. Тип данных«Boolean» обеспечивает сам 
// механизм проверок, хотя может применяться и в других задачах. Тип «Symbol» имеет крайне специфическое применение и 
// практически не применяется для хранения данных.





// ОператорыВ зависимости от характера описываемых действий, операторы и операции разделяют на группы. Например, группа 
// арифметических операторов отвечает за основные математические действия, группа логических операторов — за сравнения и
//  отношения, и т.д.



// Арифметические операторы одним из основных способов обработки данных является их математическое преобразование. 
/*
Сложение                       x + y
Вычитание                      x — y
Умножение                      x * y  
...
// Если необходимо проверять отношения для чисел, но нет уверенности, что переменные хранят их именно в числовом 
// представлении, то применяется прием добавления символа «+» перед переменной. 
console.log(x = '10');
console.log(y = '3');
console.log(y > x);
console.log(+y > +x);
...
// Объект arguments
/*Одним из главных преимуществ функций является возможность многократного использования одного и того же кода
для различных значений (аргументов), а также объединение этого кода под логичным и понятным именем.
В ряде практических задач возникает необходимость работы не только с различными значениями аргументов,
но и с разным их количеством.
 */
...
/* Свойство length - отвечает за количество переданных в функцию аргументов. 
Свойство "length" устанавливается каждый раз с новым вызовом функции и позволяет
последовательно перебрать значения аргументов, например, при помощи цикла - счетчика "for"
*/
...
logArgument("text")
/*Подобным образом в любой функции можно узнать как об общем количестве переданных аргументов,
так и об их значениях. Причем, собранные в одном объекте значения аргументов дают возможность
использовать один общий цикл вместо отдельных имен для каждого параметра.
*/

/*В качестве дополнительного примера реализуем функцию,
определяющую максимальное значение из переданных аргументов
*/
function max() {
...
y = max(1, 22, 353, 145)
console.log(y)
/*В первой строке проверяем, есть ли у нас вообще аргументы.
Если размер объекта равен нулю (arguments. length == 0) возвращаем значение «undefined».
Обратите внимание, что команда «return» завершит работу функции, если условие выполнится.
Это позволяет нам не писать «else» перед последующими командами.
Далее применяет стандартный алгоритм поиска максимума: в переменную «ret» записываем значение первого аргумента.
Затем циклически сравниваем это значение с каждым из аргументов и,
если значение аргумента окажется большим, чем хранимое, то заменяем это хранимое значение на новое.
По завершению цикла возвращаем полученный в переменной «ret» результат.
*/


// Особенности функций в JavaScript
/*Следующий код функции иллюстрирует описанные особенности. Пример плохой код */
function parity(x) {
    if (x % 2 == 0)
        parity = "even";
    else
...
/*После объявления функции она будет доступна под именем «parity».
Однако, после первого вызова функции с любым аргументом,
имя «parity» будет переопределено телом функции как переменная и,
вместо функции, она будет хранить строку «even» или «odd» в зависимости от переданного значения «х».
В JavaScript следует крайне внимательно следить за выбором имен переменных и функций!
*/

/* Область видимости переменной - называют участок программы,
 в котором возможен доступ к данной переменной по ее имени */
function logArgumen() {
    for (i = 0; i < arguments.length; i++)
        console.log("argument" + (i + 1) + " = " + arguments[i])
}
console.log(logArgumen())
/*
Объясняется это тем, что для организации цикла в функции «logArguments» используется такая же переменная «i»,
как и для цикла, который обеспечивает повторный вызов этой функции в консоли.
*/
for (i = 0; i < 3; i++)
    logArguments(i, i + 3, 2 * i, i * i)
/*Для предотвращения подобных негативных явлений в функциях могут быть описаны дополнительные переменные,
не влияющие на глобальную область видимости.Такие переменные называют локальными, а код,
в котором к ним возможен доступ — Локальной Областью Видимости(ЛОВ).  
!!!Локальные переменные имеют приоритет перед глобальными. */

// Поднятие объявлений
/*Рассмотрим пример: создадим функцию, в которой дважды выводится содержимое переменной «х» — до и после её объявления*/
function showHoisting() {
    console.log("x before declaration: " + x);
    var x = 2;
    console.log("x after declaration: " + x);
}
//!Локальная переменная var - имеет перевес от локальной на нашем примере это глобальная переменная x !!!


// Различия деклараций var, let и const
/*Переменная, объявленная оператором «let»,
не может быть повторно определена ни оператором «let»,
ни оператором «var» (в пределах данного блока).
Если переменная была ранее создана при помощи «var»,
то попытка ее повторного создания с применением «let» приведет к ошибке.
Это поведение возвращает программистам «предохранитель» повторного объявления переменных,
не работающий для декларатора «var».
const - переменную нельзя менять, но! основной принцип работы тот же как и c let переменной 
*/



// Рекурсия - называют ситуацию, в которой функция вызывает сама себя.
// Самым популярным примером для демонстрации рекурсии является функция вычисления факториала числа
